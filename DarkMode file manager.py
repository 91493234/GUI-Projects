...
class AdvancedFileManager:
    ...
    def set_dark_theme(self):
        ...
        self.fg_color = "#FFFFFF"  # Light foreground
        self.button_color = "#444444"  # Darker button
        self.entry_bg = "#3E3E3E"  # Entry background
        self.entry_fg = "#FFFFFF"  # Entry foreground
        self.highlight_color = "#0078D7"  # Highlight color
    ...
    def create_widgets(self):
        ...
        dir_label = tk.Label(dir_frame, text="Current Directory:", bg=self.bg_color, fg=self.fg_color)
        dir_label.pack(side=tk.LEFT, padx=5)

        self.dir_entry = tk.Entry(dir_frame, width=50, bg=self.entry_bg, fg=self.entry_fg, insertbackground=self.fg_color)
        self.dir_entry.pack(side=tk.LEFT, padx=5, fill=tk.X, expand=True)
        self.dir_entry.insert(0, self.current_directory)

        browse_button = tk.Button(dir_frame, text="Browse", bg=self.button_color, fg=self.fg_color,
                                  command=self.browse_directory)
        browse_button.pack(side=tk.LEFT, padx=5)

        refresh_button = tk.Button(dir_frame, text="Refresh", bg=self.button_color, fg=self.fg_color,
        ...
        # File List
        file_list_frame = tk.Frame(content_frame, bg=self.bg_color)
        file_list_frame.pack(side=tk.LEFT, fill=tk.BOTH, expand=True)

        file_list_label = tk.Label(file_list_frame, text="Files in Directory:", bg=self.bg_color, fg=self.fg_color)
        file_list_label.pack(anchor="w")

        self.file_list = tk.Listbox(file_list_frame, height=20, width=50, bg=self.entry_bg, fg=self.fg_color,
        ...
        preview_frame = tk.Frame(content_frame, bg=self.bg_color)
        preview_frame.pack(side=tk.RIGHT, fill=tk.BOTH, expand=True)

        preview_label = tk.Label(preview_frame, text="File Content Preview:", bg=self.bg_color, fg=self.fg_color)
        preview_label.pack(anchor="w")

        self.file_preview = tk.Text(preview_frame, height=20, width=50, bg=self.entry_bg, fg=self.fg_color,
                                    insertbackground=self.fg_color, state=tk.DISABLED)
        ...
        create_button = tk.Button(button_frame, text="Create File", width=15, bg=self.button_color, fg=self.fg_color,
                                  command=self.create_file)
        create_button.grid(row=0, column=0, padx=10, pady=5)

        edit_button = tk.Button(button_frame, text="Edit File", width=15, bg=self.button_color, fg=self.fg_color,
                                command=self.edit_file)
        edit_button.grid(row=0, column=1, padx=10, pady=5)

        delete_button = tk.Button(button_frame, text="Delete File", width=15, bg=self.button_color, fg=self.fg_color,
                                  command=self.delete_file)
        delete_button.grid(row=0, column=2, padx=10, pady=5)

        read_button = tk.Button(button_frame, text="Read File", width=15, bg=self.button_color, fg=self.fg_color,
                                command=self.read_file)
        read_button.grid(row=0, column=3, padx=10, pady=5)

        exit_button = tk.Button(button_frame, text="Exit", width=15, bg=self.button_color, fg=self.fg_color,
